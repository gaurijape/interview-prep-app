    ...
    <div className="space-y-3">
      {hints && hints.length > 0 && (
        <div>
          <button
            ...
            className="text-sm underline text-fgmuted"
          >
            {showHints ? "Hide hints" : "Show hints"}
          </button>
          {showHints && (
            <ul className="list-disc list-inside text-sm text-fg mt-2 space-y-1">
              {hints.map((h, i) => (
                <li key={i}>{h}</li>
              ))}
            </ul>
      ...
      <div>
        <button
          ...
          className="rounded bg-accent text-bg font-semibold text-sm px-4 py-2 rounded hover:bg-accent/80 transition-colors"
        >
